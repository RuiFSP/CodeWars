...
as you wish. Some people might prefer to reason with content, some other with percentages only. It's up
to you but you must keep it as a parameter because the tests have it as an argument.
 */
...
public class DeodorantEvaporator {
    public static int evaporator(double content, double evap_per_day, double threshold) {
        ...
        double thresholdValue = content * (threshold / 100.0);

        if (content == 0) {
            return days;
        }

        while (remaining > thresholdValue) {
            remaining *= (100.0 - evap_per_day) / 100.0;
            days++;
        }
