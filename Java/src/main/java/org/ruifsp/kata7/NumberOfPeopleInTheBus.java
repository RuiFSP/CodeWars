...
people that get on the bus (the first item) and the number of people that get off
the bus (the second item) at a bus stop.
...
public class NumberOfPeopleInTheBus {
    ...
    public static int countPassengers(ArrayList<int[]> stops) {
        ...
        for (int[] stop : stops) {
            int passengersGettingOn = stop[0];
            int passengersGettingOff = stop[1];
            totalPassengers += passengersGettingOn - passengersGettingOff;
        }
