...
public class JadenCasingStrings {
    public String toJadenCase(String phrase) {
        ...
        if (isNull(phrase) || phrase.isEmpty()) return null;

        String[] words = phrase.split(" ");

        return Arrays.stream(words).map(word -> word.substring(0,1).toUpperCase() + word.substring(1).toLowerCase()).collect(Collectors.joining(" "));
    }
