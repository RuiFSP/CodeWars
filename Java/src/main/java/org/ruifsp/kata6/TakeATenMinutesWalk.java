...
public class TakeATenMinutesWalk {
    public static boolean isValid(char[] walk) {
        ...
        // Convert the array of characters to a list of strings
        List<String> walkList = IntStream
                .range(0, walk.length)
                .mapToObj(i -> String.valueOf(walk[i]))
                .toList();

        // Initialize variables to track the number of blocks moved in each direction
        int northSouth = 0;
        int eastWest = 0;

        // Iterate through the list of directions
        for (String direction : walkList) {
            switch (direction) {
                case "n" -> northSouth++;
                case "s" -> northSouth--;
                case "e" -> eastWest++;
        ...
        // Return true if the walk took exactly 10 minutes and the person is back at their starting point
        return walkList.size() == 10 && northSouth == 0 && eastWest == 0;
